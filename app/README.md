...
Every pushed change builds the installer and Scanner APK as a draft release.
The workflow calculates the next patch version and injects it with the repository
into the app's update checker; local builds keep the baseline version in
`pubspec.yaml`.
...
> They are: `windows/` and `linux/` are tracked, and `windows/CMakeLists.txt`
